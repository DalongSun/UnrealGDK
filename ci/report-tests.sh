...
pushd "$(dirname "$0")"
    ...
    if [[ -f "${TEST_RESULTS_FILE}" ]]; then
        ...
        echo "Test results in a nicer format can be found <a href='artifact://${TEST_RESULTS_FILE}'>here</a>."| buildkite-agent annotate \
    ...
    UPLOAD_OUTPUT=$(buildkite-agent artifact upload ci/TestResults)
    if [ $? -neq 0 ]; then
