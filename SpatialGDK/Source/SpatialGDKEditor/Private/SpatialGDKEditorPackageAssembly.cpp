...
bool FSpatialGDKPackageAssembly::NextStep()
{
	...
	if (Steps.Dequeue(Target))
	{
		...
		case EPackageAssemblyStep::BUILD_SERVER:
			AsyncTask(ENamedThreads::GameThread, [this]() {
				BuildAssembly(
					FString::Printf(TEXT("%sServer"), FApp::GetProjectName()), LinuxPlatform, CloudDeploymentConfiguration.BuildConfiguration, CloudDeploymentConfiguration.BuildServerExtraArgs);
			});
			break;
		...
		case EPackageAssemblyStep::BUILD_SIMULATED_PLAYERS:
			AsyncTask(ENamedThreads::GameThread, [this]() {
				BuildAssembly(FString::Printf(TEXT("%sSimulatedPlayer"), FApp::GetProjectName()),
							  LinuxPlatform,
							  CloudDeploymentConfiguration.BuildConfiguration,
							  CloudDeploymentConfiguration.BuildSimulatedPlayerExtraArgs);
			});
			break;
	...
	else
	{
		AsyncTask(ENamedThreads::GameThread, [this]() {
			...
			if (Status == EPackageAssemblyStatus::ASSEMBLY_EXISTS)
			{
				FMessageDialog::Open(
					EAppMsgType::Ok,
					LOCTEXT("AssemblyExists_Error",
							"The assembly with the specified name has previously been uploaded. Enable the 'Force Overwrite on Upload' option in the Cloud Deployment dialog to overwrite the existing "
							"assembly or specify a different assembly name."));
			}
			else if (Status == EPackageAssemblyStatus::BAD_PROJECT_NAME)
			{
				FMessageDialog::Open(
					...
					LOCTEXT("BadProjectName_Error",
							"The project name appears to be incorrect or you do not have permissions for this project. You can edit the project name from the Cloud Deployment dialog."));
			}
...
void FSpatialGDKPackageAssembly::ShowTaskStartedNotification(const FString& NotificationText)
{
	...
	Info.ButtonDetails.Add(FNotificationButtonInfo(LOCTEXT("PackageAssemblyTaskCancel", "Cancel"),
