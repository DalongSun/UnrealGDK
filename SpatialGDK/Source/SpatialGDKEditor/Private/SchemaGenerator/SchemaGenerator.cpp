...
FActorSpecificSubobjectSchemaData GenerateSchemaForStaticallyAttachedSubobject(FCodeWriter& Writer,
...
void GenerateSubobjectSchemaForActor(
	FComponentIdGenerator& IdGenerator, UClass* ActorClass, TSharedPtr<FUnrealType> TypeInfo, FString SchemaPath, FActorSchemaData& ActorSchemaData, const FActorSchemaData* ExistingSchemaData)
{
	FCodeWriter Writer;
	...
	for (auto& It : Subobjects)
	{
		...
		if (SchemaGeneratedClasses.Contains(SubobjectClass))
		{
			...
			SubobjectData = GenerateSchemaForStaticallyAttachedSubobject(
				Writer, IdGenerator, UnrealNameToSchemaComponentName(SubobjectTypeInfo->Name.ToString()), SubobjectTypeInfo, SubobjectClass, ActorClass, 0, ExistingSubobjectSchemaData);
		}
		else
		{
			continue;
...
void GenerateSubobjectSchema(FComponentIdGenerator& IdGenerator, UClass* Class, TSharedPtr<FUnrealType> TypeInfo, FString SchemaPath)
{
	...
	for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
	{
		...
		if (Group == REP_MultiClient && Class->IsChildOf<UActorComponent>())
		{
			...
			{
				UE_LOG(LogSchemaGenerator,
					   Error,
					   TEXT("Did not find ActorComponent->bReplicates at field %d for class %s. Modifying the base Actor Component class is currently not supported."),
	...
	if (ExistingSchemaData != nullptr && !ExistingSchemaData->GeneratedSchemaName.IsEmpty() && ExistingSchemaData->GeneratedSchemaName != ClassPathToSchemaName[Class->GetPathName()])
	{
		UE_LOG(LogSchemaGenerator,
			   Error,
			   TEXT("Saved generated schema name does not match in-memory version for class %s - schema %s : %s"),
			   *Class->GetPathName(),
			   *ExistingSchemaData->GeneratedSchemaName,
			   *ClassPathToSchemaName[Class->GetPathName()]);
	...
	for (EReplicatedPropertyGroup Group : GetAllReplicatedPropertyGroups())
	{
		...
		if (Group == REP_MultiClient && Class->IsChildOf<AActor>())
		{
			...
			if (!(ExpectedReplicatesPropData.IsValid() && ExpectedReplicatesPropData->Property == ReplicatesProp))
			{
				UE_LOG(LogSchemaGenerator,
					   Error,
					   TEXT("Did not find Actor->bTearOff at field %d for class %s. Modifying the base Actor class is currently not supported."),
...
void GenerateRPCEndpointsSchema(FString SchemaPath)
{
	...
	GenerateRPCEndpoint(Writer,
						TEXT("ClientEndpoint"),
						SpatialConstants::CLIENT_ENDPOINT_COMPONENT_ID,
						{ ERPCType::ServerReliable, ERPCType::ServerUnreliable },
