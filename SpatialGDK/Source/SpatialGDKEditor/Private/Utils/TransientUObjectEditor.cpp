...
UTransientUObjectEditor* UTransientUObjectEditor::LaunchTransientUObjectEditor(const FText& EditorName, UClass* ObjectClass, TSharedPtr<SWindow> ParentWindow)
{
	...
	TSharedRef<SWindow> NewSlateWindow = SNew(SWindow).Title(EditorName)[SNew(SBorder).BorderImage(FEditorStyle::GetBrush(TEXT("PropertyWindow.WindowBorder")))[VBoxBuilder]];
