...
uint32 GetWorkerCountFromWorldSettings(const UWorld& World)
{
	...
	else
	{
		...
		FIntPoint Dimension;
		if (!EditorModule.GetLBStrategyExtensionManager().GetDefaultLaunchConfiguration(
				WorldSettings->DefaultLayerLoadBalanceStrategy->GetDefaultObject<UAbstractLBStrategy>(), LoadBalancingStrat, Dimension))
		{
			UE_LOG(LogSpatialGDKDefaultLaunchConfigGenerator,
				   Error,
				   TEXT("Could not get the default SpatialOS Load balancing strategy from %s"),
				   *WorldSettings->DefaultLayerLoadBalanceStrategy->GetName());
			NumWorkers += 1;
		}
		else
		{
...
bool TryGetLoadBalancingStrategyFromWorldSettings(const UWorld& World, UAbstractRuntimeLoadBalancingStrategy*& OutStrategy, FIntPoint& OutWorldDimension)
{
	...
	if (!EditorModule.GetLBStrategyExtensionManager().GetDefaultLaunchConfiguration(
			WorldSettings->DefaultLayerLoadBalanceStrategy->GetDefaultObject<UAbstractLBStrategy>(), OutStrategy, OutWorldDimension))
	{
		...
		if (*EnableChunkInterest == TEXT("true"))
		{
			const EAppReturnType::Type Result = FMessageDialog::Open(
				...
				LOCTEXT(
					...
					"The legacy flag \"enable_chunk_interest\" is set to true in the generated launch configuration. Chunk interest is not supported and this flag needs to be set to false.\n\nDo you "
					"want to configure your launch config settings now?"));
