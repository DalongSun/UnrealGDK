 ...
 * The test also REQUIRES the presence of a ADormancyTestActor (this actor is initially dormant) in the level where it is placed.
 ...
 *  - Cleanup:
 *    - No cleanup required, as the actor is deleted as part of the test. Note that the actor exists in the world if other tests are run before this one.
...
void ADormancyAndTombstoneTest::BeginPlay()
{
	...
	{ // Step 1 - Set TestIntProp to 1.
		AddServerStep(TEXT("ServerSetTestIntPropTo1"), 1, nullptr, [](ASpatialFunctionalTest* NetTest) {
			...
			{
				Counter++;
				NetTest->AssertEqual_Int(Iter->NetDormancy, DORM_Initial, TEXT("Dormancy on ADormancyTestActor (should be DORM_Initial)"), NetTest);
				Iter->TestIntProp = 1;
			}
