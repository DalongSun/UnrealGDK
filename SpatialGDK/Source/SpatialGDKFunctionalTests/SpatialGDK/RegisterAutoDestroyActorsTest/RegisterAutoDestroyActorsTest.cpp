...
void ARegisterAutoDestroyActorsTestPart1::BeginPlay()
{
	...
	{ // Step 1 - Spawn Actor On Auth
		AddServerStep(TEXT("SERVER_1_Spawn"), 1, nullptr, [](ASpatialFunctionalTest* NetTest) {
			...
			for (int i = 0; i != NumVirtualWorkers; ++i)
			{
				...
				NetTest->AssertTrue(IsValid(Character),
									FString::Printf(TEXT("Spawned ACharacter in worker %s"), *NetTest->GetFlowController(ESpatialFunctionalTestFlowControllerType::Server, i + 1)->GetDisplayName()));
	...
	{ // Step 2 - Check If Clients have it
		AddClientStep(
			TEXT("CLIENT_ALL_CheckActorsSpawned"),
			FWorkerDefinition::ALL_WORKERS_ID,
			nullptr,
			nullptr,
	...
	{ // Step 3 - Destroy by second server that doesn't have authority
		AddServerStep(
			TEXT("SERVER_2_RegisterAutoDestroyActors"),
			2,
