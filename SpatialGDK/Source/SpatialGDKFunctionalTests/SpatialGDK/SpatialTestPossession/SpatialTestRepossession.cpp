...
void ASpatialTestRepossession::BeginPlay()
{
	...
	AddServerStep(TEXT("SpatialTestRepossessionServerSetupStep"), 1, nullptr, nullptr, [](ASpatialFunctionalTest* NetTest, float DeltaTime) {
		...
		for (ASpatialFunctionalTestFlowController* FlowController : Test->GetFlowControllers())
		{
			...
			ATestPossessionPawn* TestPawn = Test->GetWorld()->SpawnActor<ATestPossessionPawn>(FVector(0.0f, YToSpawnAt, 50.0f), FRotator::ZeroRotator, FActorSpawnParameters());
	...
	auto ClientCheckPossessionTickLambda = [](ASpatialFunctionalTest* NetTest, float DeltaTime) {
		...
		Test->AssertTrue(Test->Controllers.Num() == Test->TestPawns.Num(), TEXT("Number of players is equal to the number of pawns spawned"), LocalFlowController);
